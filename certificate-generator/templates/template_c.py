...
def generate_synthetic_diploma(
    ...
    font_script_path: str | None = None,
    font_body_path: str | None = None,
    placeholders: dict | None = None,
    left_signature_png: str | None = None,    # President / Vice-Chancellor  (sig1)
    right_signature_png: str | None = None,   # Registrar                    (sig2)
    dean_signature_png: str | None = None,    # Dean, Faculty of Engineering (sig3)
    seal_png: str | None = None,              # University seal
    signature_opacity: float = 1.0,
    seal_opacity: float = 1.0,
):
    """
    Layout matches the reference diploma image exactly:

    ┌─────────────────────────────────────────────────────────────────────┐
    │               INSTITUTION  (blackletter, large)                     │
    │          The Senate confirms that  (italic body)                    │
    │               Student Name  (blackletter, large)                    │
    │            is admitted to the degree  (italic body)                 │
    │           Bachelor of Applied Science  (blackletter)                │
    │         Honours … Option  (italic body, multiline)                  │
    │        with all associated rights…  (italic body)                   │
    │                                                                     │
    │  [sig1]           [  SEAL  ]           [sig3]                       │
    │  PRESIDENT AND    (large, centered)    DEAN, FACULTY OF             │
    │  VICE-CHANCELLOR                       ENGINEERING                  │
    │                                                                     │
    │  [sig2]                                THIS FIFTH DAY OF JUNE       │
    │  REGISTRAR                             TWO THOUSAND AND…            │
    └─────────────────────────────────────────────────────────────────────┘

    Three columns are centred at W*0.20, W*0.50, W*0.80 so they are
    perfectly equidistant with the seal exactly in the middle.
    """
    ...
    y = draw_multiline_centered(
        draw,
        [placeholders["PROGRAM"], placeholders["HONOURS"], placeholders["RIGHTS"]],
        y, body_font, W, line_gap=int(22 * base), fill=text_color,
    )

    # ── bottom section ───────────────────────────────────────────────────
    #
    # Column centres:
    #   left   = 20 % W   (President + Registrar stacked)
    #   center = 50 % W   (Seal)
    #   right  = 80 % W   (Dean)
    #
    # Signature image box: sig_col_w wide, sig_box_h tall
    # Seal image box     : seal_size × seal_size, centred on seal_cx
    #
    left_cx  = int(W * 0.20)
    seal_cx  = W // 2
    right_cx = int(W * 0.80)

    sig_col_w   = int(W * 0.22)    # signature image width
    sig_box_h   = int(160 * base)  # signature image height  (bigger than before)
    title_gap   = int(20  * base)
    between_gap = int(45  * base)  # gap between sig1-title and sig2-image

    seal_size   = int(900 * base)  # seal diameter  (1.5-2x larger)

    # All three signature rows start at the same Y
    sig_row_y = int(H * 0.64)

    # Convert column centres → left edges
    left_col_x  = left_cx  - sig_col_w // 2
    right_col_x = right_cx - sig_col_w // 2

    # ── Sig 1: President / Vice-Chancellor ───────────────────────────────
    paste_png(img, left_signature_png,
    ...
    # ── Sig 2: Registrar (stacked below sig 1) ───────────────────────────
    sig2_row_y = sig1_title_y + title1_h + between_gap

    paste_png(img, right_signature_png,
    ...
    # ── Seal ─────────────────────────────────────────────────────────────
    # Vertically center the seal across the full left signature block
    # (top of sig1 image to bottom of sig2 title)
    ...
    seal_y = sig_row_y + (sig_block_h - seal_size) // 2  # vertically centered
    ...
    if seal_png and Path(seal_png).exists():
        ...
        draw.text((cx - (bb[2]-bb[0])//2, cy - (bb[3]-bb[1])//2), lbl, fill=text_color, font=fb)

    # ── Sig 3: Dean, Faculty of Engineering ──────────────────────────────
    paste_png(img, dean_signature_png,
    ...
    # ── Date: below dean title, aligned to dean column ───────────────────
...
if __name__ == "__main__":
    out = generate_synthetic_diploma(
        out_path="synthetic_diploma_15.5x10.5.png",
        dpi=300,
        page_inches=(15.5, 10.5),
        font_title_path="CloisterBlack.ttf",
        font_body_path="Book Antiqua Italic.ttf",

        left_signature_png=str(BASE_DIR.parent / "assets" / "signatures" / "sig1.png"),
        right_signature_png=str(BASE_DIR.parent / "assets" / "signatures" / "sig2.png"),
        dean_signature_png=str(BASE_DIR.parent / "assets" / "signatures" / "sig3.png"),
        seal_png=str(BASE_DIR.parent / "assets" / "seals" / "sealmw.png"),
        ...
        placeholders={
            ...
            "PROGRAM":          "Honours Computer Engineering/Management Sciences Option –",
            "HONOURS":          "Co-operative Program",
            "RIGHTS":           "(With Distinction)",
            "SIGN_LEFT_TITLE":  "President and Vice-Chancellor",
            "SIGN_RIGHT_TITLE": "Registrar",
            "SIGN_DEAN_TITLE":  "Dean, Faculty of Engineering",
            "DATE_LINE":        "THIS FIFTH DAY OF JUNE, TWO THOUSAND AND TWENTY-THREE",
        },
    )
    print("Saved:", out)