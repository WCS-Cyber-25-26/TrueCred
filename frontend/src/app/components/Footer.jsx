"use client";

import Link from "next/link";

function ShieldIcon() {
  return (
    <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
      <path
        d="M12 2L3 7V12C3 16.55 7.08 20.74 12 22C16.92 20.74 21 16.55 21 12V7L12 2Z"
        stroke="#60a5fa"
        strokeWidth="1.5"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <path
        d="M9 12L11 14L15 10"
        stroke="#60a5fa"
        strokeWidth="1.5"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </svg>
  );
}

function FooterLink({ href, children, onClick }) {
  return (
    <li>
      <Link
        href={href}
        onClick={onClick}
        className="text-sm transition-colors duration-200"
        style={{ color: "#475569" }}
        onMouseEnter={(e) => (e.currentTarget.style.color = "#cbd5e1")}
        onMouseLeave={(e) => (e.currentTarget.style.color = "#475569")}
      >
        {children}
      </Link>
    </li>
  );
}

function SocialIcon({ href, label, children }) {
  return (
    <a
      href={href}
      aria-label={label}
      className="w-8 h-8 flex items-center justify-center rounded-md transition-all duration-200"
      style={{
        backgroundColor: "rgba(13,31,60,0.8)",
        border: "1px solid rgba(30,58,138,0.35)",
        color: "#475569",
      }}
      onMouseEnter={(e) => {
        e.currentTarget.style.borderColor = "rgba(59,130,246,0.5)";
        e.currentTarget.style.color = "#60a5fa";
      }}
      onMouseLeave={(e) => {
        e.currentTarget.style.borderColor = "rgba(30,58,138,0.35)";
        e.currentTarget.style.color = "#475569";
      }}
    >
      {children}
    </a>
  );
}

export default function Footer() {
  const scrollToSection = (id) => {
    const element = document.getElementById(id);
    if (element) element.scrollIntoView({ behavior: "smooth" });
  };

  return (
    <footer style={{ backgroundColor: "#020817", borderTop: "1px solid rgba(30,58,138,0.25)" }}>
      <div className="max-w-[1440px] mx-auto px-6 md:px-12 pt-14 pb-8">
        <div className="grid grid-cols-1 gap-10 md:grid-cols-4 mb-12">

          {/* Brand */}
          <div className="md:col-span-1">
            <Link
              href="/"
              onClick={() => scrollToSection("home")}
              className="inline-flex items-center gap-2 mb-4"
            >
              <ShieldIcon />
              <span
                className="font-display font-bold text-lg tracking-tight text-white"
              >
                TrueCred
              </span>
            </Link>
            <p className="text-sm leading-relaxed" style={{ color: "#475569" }}>
              AI-powered, blockchain-secured credential verification for universities and enterprise HR.
            </p>

            {/* Social icons */}
            <div className="flex gap-2 mt-6">
              <SocialIcon href="#linkedin" label="LinkedIn">
                <svg width="14" height="14" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z" />
                </svg>
              </SocialIcon>
              <SocialIcon href="#twitter" label="Twitter">
                <svg width="14" height="14" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M23 3a10.9 10.9 0 01-3.14 1.53 4.48 4.48 0 00-7.86 3v1A10.66 10.66 0 013 4s-4 9 5 13a11.64 11.64 0 01-7 2c9 5 20 0 20-11.5a4.5 4.5 0 00-.08-.83A7.72 7.72 0 0023 3z" />
                </svg>
              </SocialIcon>
              <SocialIcon href="#github" label="GitHub">
                <svg width="14" height="14" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" />
                </svg>
              </SocialIcon>
            </div>
          </div>

          {/* Product */}
          <div>
            <h4
              className="font-display font-bold text-sm uppercase tracking-widest mb-4"
              style={{ color: "#94a3b8" }}
            >
              Product
            </h4>
            <ul className="space-y-3">
              <FooterLink href="/#features" onClick={() => scrollToSection("features")}>Features</FooterLink>
              <FooterLink href="/#pricing">Pricing</FooterLink>
              <FooterLink href="/#api">API Docs</FooterLink>
              <FooterLink href="/#security">Security</FooterLink>
            </ul>
          </div>

          {/* Company */}
          <div>
            <h4
              className="font-display font-bold text-sm uppercase tracking-widest mb-4"
              style={{ color: "#94a3b8" }}
            >
              Company
            </h4>
            <ul className="space-y-3">
              <FooterLink href="/about">About Us</FooterLink>
              <FooterLink href="#careers">Careers</FooterLink>
              <FooterLink href="#blog">Blog</FooterLink>
            </ul>
          </div>

          {/* Contact */}
          <div>
            <h4
              className="font-display font-bold text-sm uppercase tracking-widest mb-4"
              style={{ color: "#94a3b8" }}
            >
              Contact
            </h4>
            <ul className="space-y-3">
              <li className="text-sm" style={{ color: "#475569" }}>verify@truecred.io</li>
              <li className="text-sm" style={{ color: "#475569" }}>+1 (800) TRU-CRED</li>
              <li className="text-sm" style={{ color: "#475569" }}>London, ON</li>
            </ul>
          </div>
        </div>

        {/* Bottom bar */}
        <div
          className="flex flex-col gap-4 pt-8 sm:flex-row sm:items-center sm:justify-between"
          style={{ borderTop: "1px solid rgba(30,58,138,0.2)" }}
        >
          <p className="text-xs" style={{ color: "#334155" }}>
            © 2026 TrueCred. All rights reserved.
          </p>
          <div className="flex gap-6">
            {["Privacy Policy", "Terms of Service", "Cookie Policy"].map((label) => (
              <Link
                key={label}
                href={`/#${label.toLowerCase().replace(/ /g, "-")}`}
                className="text-xs transition-colors duration-200"
                style={{ color: "#334155" }}
                onMouseEnter={(e) => (e.currentTarget.style.color = "#64748b")}
                onMouseLeave={(e) => (e.currentTarget.style.color = "#334155")}
              >
                {label}
              </Link>
            ))}
          </div>
        </div>
      </div>
    </footer>
  );
}
